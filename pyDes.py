...
class _baseDes(object):
    def __init__(self, mode=ECB, IV=None, pad=None, padmode=PAD_NORMAL):
        ...
        if IV and len(IV) != self.block_size:
            raise ValueError(
                "Invalid Initial Value (IV), must be a multiple of " +
    ...
    def _padData(self, data, pad, padmode):
        ...
        if padmode == PAD_NORMAL:
            ...
            if not pad:
                raise ValueError(
                    "Data must be a multiple of " + str(self.block_size) +
...
class des(_baseDes):
    ...
    __ip = [
        ...
        45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7, 56, 48, 40, 32,
        24, 16, 8, 0, 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20,
        ...
        if len(data) % self.block_size != 0:
            ...
            if not self.getPadding():
                ...
                data += (self.block_size -
                         (len(data) % self.block_size)) * self.getPadding()
